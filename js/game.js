...
for (let i = 0; i < 1200 && interactives.filter(t => t.type === 'tree').length < 110; i++){
  ...
  const y = heightAt(x, z);
  if (y < WATER_Y + 0.8 || riverDist(x, z) < 8 || Math.hypot(x - 10, z - 26) < 9 || Math.hypot(x - 58, z + 38) < 7 || Math.hypot(x + 36, z - 42) < 7 || Math.hypot(x + 0.75, z + 22) < 8 || Math.hypot(x + 52, z - 16) < 8 || Math.hypot(x - 24, z - 56) < 8) continue;
...
function animate(){
  ...
  if (playing){
    ...
    const it = nearest();
    const plot = nearestPlot();
    const hereName = atShadePool(hero.position.x, hero.position.z) ? 'The Shade Pool' : atSlowBend(hero.position.x, hero.position.z) ? 'The Slow Bend' : currentPlace(hero.position.x, hero.position.z);
