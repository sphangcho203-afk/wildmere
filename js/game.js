...
for (let i = 0; i < 1200 && interactives.filter(t => t.type === 'tree').length < 110; i++){
  ...
  const y = heightAt(x, z);
  if (y < WATER_Y + 0.8 || riverDist(x, z) < 8 || Math.hypot(x - 10, z - 26) < 9 || Math.hypot(x - 58, z + 38) < 7 || Math.hypot(x + 36, z - 42) < 7 || Math.hypot(x + 0.75, z + 22) < 8 || Math.hypot(x + 52, z - 16) < 8 || Math.hypot(x - 24, z - 56) < 8 || Math.hypot(x - 78, z - 22) < 8 || Math.hypot(x + 22, z - 28) < 7 || Math.hypot(x + 8, z + 48) < 7) continue;
  ...
  if (playing){
    ...
    if (resting || notebookOpen){
      ...
      if (!foundHollow && atWindHollow(hero.position.x, hero.position.z)){
        foundHollow = true; toast('The Wind Hollow. A bowl of stone. The cloth keeps moving.');
      }
      ...
      if (!foundPool && atShadePool(hero.position.x, hero.position.z)){
        foundPool = true; toast('The Shade Pool. Still water. A log to sit on.');
      }
      if (!foundOak && atSplitOak(hero.position.x, hero.position.z)){
        foundOak = true; toast('The Split Oak. The trunk opened years ago and kept growing.');
      }
      if (!foundGate && atStillGate(hero.position.x, hero.position.z)){
        foundGate = true; toast('The Still Gate. Two posts. The lintel fell and stayed.');
      }
      if (!foundWash && atWashRock(hero.position.x, hero.position.z)){
        foundWash = true; toast('The Wash Rock. Cloth on a post. The stream is close.');
      }
      if (!foundLark && atLarkPost(hero.position.x, hero.position.z)){
        foundLark = true; toast('The Lark Post. Thin slats. The air has something to tap.');
      }
    ...
    if (windHollow && windHollow.ribbon){
      const wr = raining ? 1.7 : 1;
      windHollow.ribbon.rotation.z = Math.sin(clock.elapsedTime * 2.1 * wr) * 0.28 * wr;
      windHollow.ribbon.rotation.y = Math.sin(clock.elapsedTime * 1.4) * 0.16;
    }
    if (reedStep && reedStep.reeds){
      const wr = raining ? 1.35 : 1;
      for (let i = 0; i < reedStep.reeds.length; i++){
        reedStep.reeds[i].rotation.z = Math.sin(clock.elapsedTime * 1.6 * wr + i * 0.4) * 0.07 * wr;
      }
    }
    if (shadePool && shadePool.reeds){
      const wr = raining ? 1.35 : 1;
      for (let i = 0; i < shadePool.reeds.length; i++){
        shadePool.reeds[i].rotation.z = Math.sin(clock.elapsedTime * 1.4 * wr + i * 0.35) * 0.08 * wr;
      }
    }
    if (splitOak && splitOak.leaves){
      const wr = raining ? 1.4 : 1;
      for (let i = 0; i < splitOak.leaves.length; i++){
        splitOak.leaves[i].rotation.y = Math.sin(clock.elapsedTime * 0.55 * wr + i * 0.7) * 0.04 * wr;
      }
    }
    if (washRock && washRock.cloths){
      const wr = raining ? 1.6 : 1;
      for (let i = 0; i < washRock.cloths.length; i++){
        washRock.cloths[i].rotation.z = Math.sin(clock.elapsedTime * 1.8 * wr + i) * 0.22 * wr;
      }
    }
    if (larkPost && larkPost.slats){
      ...
      for (let i = 0; i < larkPost.slats.length; i++){
        ...
        larkPost.slats[i].rotation.x = Math.sin(clock.elapsedTime * 1.6 + i) * 0.08;
      }
    }
    if (slowBend.fish){
    ...
    const it = nearest();
    const plot = nearestPlot();
    const hereName = atLarkPost(hero.position.x, hero.position.z) ? 'The Lark Post' : atWashRock(hero.position.x, hero.position.z) ? 'The Wash Rock' : atStillGate(hero.position.x, hero.position.z) ? 'The Still Gate' : atSplitOak(hero.position.x, hero.position.z) ? 'The Split Oak' : atShadePool(hero.position.x, hero.position.z) ? 'The Shade Pool' : atSlowBend(hero.position.x, hero.position.z) ? 'The Slow Bend' : currentPlace(hero.position.x, hero.position.z);
    ...
    if (pr){
      ...
      else if (atSplitOak(hero.position.x, hero.position.z)) pr.textContent = 'Split Oak — two trunks, a low bench';
      else if (atStillGate(hero.position.x, hero.position.z)) pr.textContent = 'Still Gate — posts and a fallen lintel';
      else if (atWashRock(hero.position.x, hero.position.z)) pr.textContent = 'Wash Rock — paddle, bucket, cloth';
      else if (atLarkPost(hero.position.x, hero.position.z)) pr.textContent = 'Lark Post — slats tap in the air';
