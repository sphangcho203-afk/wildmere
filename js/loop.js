...
for (let i = 0; i < 1200 && interactives.filter(t => t.type === 'tree').length < 110; i++){
  ...
  const y = heightAt(x, z);
  if (y < WATER_Y + 0.8 || riverDist(x, z) < 8 || Math.hypot(x - 10, z - 26) < 9 || Math.hypot(x - 58, z + 38) < 7 || Math.hypot(x + 36, z - 42) < 7 || Math.hypot(x + 0.75, z + 22) < 8 || Math.hypot(x + 52, z - 16) < 8 || Math.hypot(x - 24, z - 56) < 8 || Math.hypot(x + 22, z - 28) < 7 || Math.hypot(x + 8, z + 48) < 7 || Math.hypot(x - 52, z - 38) < 7 || Math.hypot(x + 28, z + 44) < 7 || Math.hypot(x - 38, z - 22) < 7 || Math.hypot(x - 24, z + 32) < 8 || Math.hypot(x - 22, z - 54) < 8 || Math.hypot(x - 38, z - 36) < 7 || Math.hypot(x - 4, z + 6) < 7 || Math.hypot(x - 24, z - 4) < 7 || Math.hypot(x + 14, z + 6) < 7 || Math.hypot(x - 8, z - 38) < 7) continue;
...
bootTick({
  ...
  fernStair, larkPost, eveningBell, washRock, windHollow, rowanLean, willowDip, honeyStone, thistleSeat, cloverPad, daisyRing, rushNest, birchShelf, alderNook,
  skyU, sun, dir, hemi,
  WATER_Y, foundNotes, interactives, fires, plots, player,
  atQuietWell, atShadePool, atSlowBend, atEveningBell, atRowanLean, atWillowDip, atHoneyStone, atThistleSeat, atCloverPad, atDaisyRing, atRushNest, atBirchShelf, atAlderNook,
  getPlaying: () => playing,
